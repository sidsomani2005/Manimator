...
def main_prompt(prompt: str):
    system_instruction = """
    You are an assistant that generates a JSON response consisting of 'images', 'text', and 'info' fields to explain a concept provided in the user prompt in as simple a way as possible with as many details as possible.
...
def manimator(data: str):
    ...
    **Your Task:**
    ...
    - The 'info' field is a dictionary with keys being the timestamps from the generated SRT file and values being detailed steps on how to draw and animate the concept being explained at that timestamp.


    - **Output:** Generate Manim code that:
    - Creates animations illustrating the concepts described in the `text` field and using the corresponding steps in the 'info' field at the timestamp in 'text'.
    ...
    - Ensure that the animations follow the steps detailed in the 'info' field and align with the voiceover timestamps.
    - When generating animations, adhere to mathematical logic and physics principles.
...
#     mp = main_prompt("explain how multiplication works visually")
#     print(mp)
#     print(manimator(mp))
